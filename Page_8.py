...
from General_Functions import return_table, build_query, search_by_button, create_entry_user
from User import validate_user, role_table, user_collection_name, user_codeID_label
from Category import collection_name, category_codeID_label, return_all_categories, make_category, \
    category_name_label, category_createdAt_label, update_category, validate_category, delete_category, \
    full_entry_category, create_category
...
def page_8_layout():
    ...
    # Page 8: Category search / management page.
    ...
    if status:
        ...
        if entry[0]["Role"] == role_table["Administrator"]:
            ...
            all_entries = return_all_categories({})
            selected_entry = search(all_entries)

            # Step 2e: Alter, Add and Delete categories
            alter_category(selected_entry, all_entries)
            add_category()
            remove_category(selected_entry, all_entries)
...
def search_form(entries):
    ...
    with st.container(border=True):
        ...
        attribute = "CodeID"
        codeID = search_by_button(category_codeID_label, return_table(attribute, entries, None, False), collection_name, attribute, "Search")

        attribute = "CreatedAt"
        createdAt = search_by_button(category_createdAt_label, return_table(attribute, entries, None, True), collection_name, attribute, "Search")

        attribute = "UserID"
        userID = search_by_button(user_codeID_label, return_table(attribute, entries, None, False), collection_name, attribute, "Search")

        attribute = "RuleID"
        ruleID = search_by_button(rule_codeID_label, return_table(attribute, entries, None, False), collection_name, attribute, "Search")

        attribute = "Name"
        name = search_by_button(category_name_label, return_table(attribute, entries, None, False), collection_name, attribute, "Search")

        # Step 3e: Construct category-shaped object for query consistency
        query_like = make_category(codeID, createdAt, userID, name, ruleID)
        ...
        if show_result:
            ...
            return return_all_categories(query)
        else:
            return {}
...
def display_results(outcome):
    ...
    while pointer < len(outcome):
        ...
        with st.container(border=True):
            full_entry_category(outcome, pointer, True)
...
def alter_category_officially(codeID: str, name: str, userID: str, ruleID: str = None):
    # Step 1: Persist the update to the database (and capture status into session_state)
    st.session_state.error, _, st.session_state.error_status = update_category(codeID, name, userID, ruleID)
...
def alter_category(entry, entries):
    ...
    with st.container(border=True):
        ...
        if entry is None:
            attribute = "CodeID"
            codeID = search_by_button(category_codeID_label, return_table(attribute, entries, [], False),
                                      collection_name, attribute, "Alter")
        # Step 4: Otherwise, CodeID is already known from entry
        else:

            codeID = st.text_input(category_codeID_label, value=entry['CodeID'], key="alter_category_codeID",
                                   disabled=True)

        # Step 5: Pick/alter the rule for this category (pre-filled if entry exists, depending on alter_ruleID)
        ruleID = select_ruleID(entry, 1)

        # Step 6: Input for category name (pre-filled if entry exists)
        name = st.text_input(category_name_label, value=name_set, key="alter_category_name")

        # Step 7: Submit button - triggers DB update via callback, passing the collected inputs
        st.button(
            f"Update Entry {codeID}",
            use_container_width=True,
            on_click=alter_category_officially,
            args=[codeID, name, st.session_state.current_user, ruleID],
            key=f"alter_category"
        )


def add_category_officially(name: str, userID: str, ruleID: str = None):
    # Step 1: Persist the create operation to the database (and capture status into session_state)
    st.session_state.error, _, st.session_state.error_status = create_category(name, userID, ruleID)
...
def add_category():
    ...
    with st.container(border=True):
        ...
        # Step 4: Pick rule for this new category
        ruleID = select_ruleID(None, 2)

        # Step 5: Input for new category name
        name = st.text_input(category_name_label, key="add_category_name")

        # Step 6: Submit button - should create a new entry
        st.button(
            f"Add Entry",
            use_container_width=True,
            on_click=add_category_officially,
            args=[name, userID, ruleID],
            key=f"add_category"
        )


def remove_category_officially(codeID: str, userID: str):
    # Step 1: Persist the delete operation to the database (and capture status into session_state)
    st.session_state.error, _, st.session_state.error_status = delete_category(codeID, userID)
...
def remove_category(entry, entries):
    ...
    with st.container(border=True):
        ...
        # Step 4: Otherwise show CodeID as read-only (already known)
            ...
            codeID = st.text_input(
                category_codeID_label,
                value=entry['CodeID'],
                key="delete_category_codeID",
                disabled=True
            )

        # Step 5: Validate that this category exists / can be deleted, and fetch full entry data
        ...
        if category_status:
            full_entry_category(category_entry, 0, False)

            # Step 7: Confirm delete action with a button; calls delete callback with CodeID and current user
            st.button(
                f"Delete Entry {codeID}",
                use_container_width=True,
                on_click=remove_category_officially,
                args=[codeID, st.session_state.current_user],
                key=f"delete_category"
            )


def manage_categories():

    # Step 5a: Retrieve all categories belonging to the current user
    entries = return_all_categories({"UserID": st.session_state.current_user})

    # Step 5b: Present the search/action interface for category management
    option = search_by_button(category_name_label,
                              return_table("Name", entries,
                                           ["Add", "Show All"], False), collection_name, "Name",
                              "Search")

    # Step 5c: Display all categories if the user selects "Show All"
    if option == "Show All":
        display_results(entries)

    # Step 5d: Start the category creation workflow if the user selects "Add"
    elif option == "Add":
        add_category()

    # Step 5e: Retrieve the selected category and allow modification or removal
    else:
        category_entry = return_all_categories({"UserID": st.session_state.current_user, "Name": option})

        # Step 5f: Ensure the selected category exists before altering or removing it
        if len(category_entry) >= 1:
            alter_category(category_entry[0], category_entry)
            remove_category(category_entry[0], category_entry)
