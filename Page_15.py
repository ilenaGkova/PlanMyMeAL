...
from General_Functions import search_by_button, build_query, return_table, create_entry_user
from Menu import menu
from MongoDB_General_Functions import role_table
from UnitType import collection_name, return_all_unit_types, make_unit_type, unit_type_codeID_label, \
    unit_type_createdAt_label, unit_type_name_label, full_entry_unit_type, update_unit_type, create_unit_type, \
    delete_unit_type, validate_unit_type
...
def page_15_layout():
    ...
    # Page 15: Unit Type search / management page.
    ...
    if status:
        ...
        if entry[0]["Role"] == role_table["Administrator"]:
            ...
            all_entries = return_all_unit_types({})
            selected_entry = search(all_entries)

            # Step 2e: Alter, Add and Delete Schedules
            alter_unit_type(selected_entry, all_entries)
            add_unit_type()
            remove_unit_type(selected_entry, all_entries)
...
def search_form(entries):
    ...
    with st.container(border=True):
        ...
        attribute = "CodeID"
        codeID = search_by_button(unit_type_codeID_label, return_table(attribute, entries, None, False),
                                  collection_name, attribute, "Search")

        attribute = "CreatedAt"
        createdAt = search_by_button(unit_type_createdAt_label, return_table(attribute, entries, None, True),
                                     collection_name, attribute, "Search")
        ...
        attribute = "Name"
        name = search_by_button(unit_type_name_label, return_table(attribute, entries, None, False), collection_name,
                                attribute, "Search")

        # Step 3e: Construct UnitType-shaped object for query consistency
        query_like = make_unit_type(codeID, createdAt, userID, name)
        ...
        if show_result:
            ...
            return return_all_unit_types(query)
        else:
            return {}
...
def display_results(outcome):
    ...
    while pointer < len(outcome):
        ...
        with st.container(border=True):
            full_entry_unit_type(outcome, pointer, True)
...
def alter_unit_type_officially(codeID: str, name: str, userID: str):
    # Step 1: Persist the update to the database (and capture status into session_state)
    st.session_state.error, _, st.session_state.error_status = update_unit_type(codeID, name, userID)
...
def alter_unit_type(entry, entries):
    ...
    with st.container(border=True):
        ...
        if entry is None:
            attribute = "CodeID"
            codeID = search_by_button(unit_type_codeID_label, return_table(attribute, entries, [], False),
                                      collection_name, attribute, "Alter")
        # Step 4: Otherwise, CodeID is already known from entry
        else:

            codeID = st.text_input(unit_type_codeID_label, value=entry['CodeID'], key="alter_unit_type_codeID",
                                   disabled=True)

        # Step 5: Input for unit_type name (pre-filled if entry exists)
        name = st.text_input(unit_type_name_label, value=name_set, key="alter_unit_type_name")

        # Step 6: Submit button - triggers DB update via callback, passing the collected inputs
        st.button(
            f"Update Entry {codeID}",
            use_container_width=True,
            on_click=alter_unit_type_officially,
            args=[codeID, name, st.session_state.current_user],
            key=f"alter_unit_type"
        )


def add_unit_type_officially(name: str, userID: str):
    # Step 1: Persist the create operation to the database (and capture status into session_state)
    st.session_state.error, _, st.session_state.error_status = create_unit_type(name, userID)
...
def add_unit_type():
    ...
    with st.container(border=True):
        ...
        # Step 4: Input for new Unit Type name
        name = st.text_input(unit_type_name_label, key="add_unit_type_name")

        # Step 5: Submit button - should create a new entry
        st.button(
            f"Add Entry",
            use_container_width=True,
            on_click=add_unit_type_officially,
            args=[name, userID],
            key=f"add_unit_type"
        )


def remove_unit_type_officially(codeID: str, userID: str):
    # Step 1: Persist the delete operation to the database (and capture status into session_state)
    st.session_state.error, _, st.session_state.error_status = delete_unit_type(codeID, userID)
...
def remove_unit_type(entry, entries):
    ...
    with st.container(border=True):
        ...
        # Step 4: Otherwise show CodeID as read-only (already known)
            ...
            codeID = st.text_input(
                unit_type_codeID_label,
                value=entry['CodeID'],
                key="delete_unit_type_codeID",
                disabled=True
            )

        # Step 5: Validate that this Unit Type exists / can be deleted, and fetch full entry data
        ...
        if unit_type_status:
            full_entry_unit_type(unit_type_entry, 0, False)

            # Step 7: Confirm delete action with a button; calls delete callback with CodeID and current user
            st.button(
                f"Delete Entry {codeID}",
                use_container_width=True,
                on_click=remove_unit_type_officially,
                args=[codeID, st.session_state.current_user],
                key=f"delete_unit_type"
            )


def manage_unit_types():

    # Step 5a: Retrieve all unit types belonging to the current user
    entries = return_all_unit_types({"UserID": st.session_state.current_user})

    # Step 5b: Present the search/action interface for unit type management
    option = search_by_button(unit_type_name_label,
                              return_table("Name", entries,
                                           ["Add", "Show All"], False), collection_name, "Name",
                              "Search")

    # Step 5c: Display all unit types if the user selects "Show All"
    if option == "Show All":
        display_results(entries)

    # Step 5d: Start the unit type creation workflow if the user selects "Add"
    elif option == "Add":
        add_unit_type()

    # Step 5e: Retrieve the selected unit type and allow modification or removal
    else:
        unit_type_entry = return_all_unit_types({"UserID": st.session_state.current_user, "Name": option})

        # Step 5f: Ensure the selected unit type exists before altering or removing it
        if len(unit_type_entry)>=1:
            alter_unit_type(unit_type_entry[0], unit_type_entry)
            remove_unit_type(unit_type_entry[0], unit_type_entry)